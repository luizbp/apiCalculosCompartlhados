...
tsk.default <-
            ...
            use.log.dose = TRUE,...)
  {
    ...
    tsk(input,control,trim,conf.level,use.log.dose,...)
  }
...
tsk.data.frame <-
            ...
            use.log.dose = TRUE, ...)
  {
    ...
    if (use.log.dose && any(input$x < 0)) {
      warning("Negative doses exist in the data. ",
              "Forcing use.log.dose=FALSE.")
      use.log.dose <- FALSE
    }
    if (any(input$r < 0)) {
    ...
    names(use.log.dose) <- "calculations done using the logs of the doses?"
    attr(cint,"conf.level") <- conf.level
    ###---OUTPUT-----------------------------------------------------
    if (use.log.dose) {
      names(gsd) <- "geometric standard deviation of LD50 estimate"
      rval <- list(use.log.dose=use.log.dose,
                   trim=trim,
